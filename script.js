// ใส่ URL ที่ได้จาก Web App Deployment ใหม่ที่นี่
...
// -------------------------------------------------------------
// 🔴🔴 แก้ไขรหัสโฟลเดอร์ให้ตรงกับที่คุณให้มาแล้ว 🔴🔴
// -------------------------------------------------------------
// ID เดิมผิด แก้เป็น: 1jx5XVDpGo5MAkykNbJ08eJgW5zTVVXmV
const QR_FOLDER_ID = "1jx5XVDpGo5MAkykNbJ08eJgW5zTVVXmV"; 
// -------------------------------------------------------------

const mockMenuData = [
    { id: "M001", name: "เบียร์สิงห์", price: 65, category: "เครื่องดื่ม", image: "" },
    { id: "M002", name: "เบียร์ลีโอ", price: 60, category: "เครื่องดื่ม", image: "" },
    { id: "M003", name: "เลย์ (ห่อใหญ่)", price: 30, category: "ขนม/ของว่าง", image: "" },
    { id: "M004", name: "คาราบาวแดง", price: 10, category: "เครื่องดื่ม", image: "" },
    { id: "M005", name: "น้ำปลาทิพรส", price: 35, category: "อาหารแห้ง/เครื่องปรุง", image: "" },
    { id: "M006", name: "ไข่ไก่ (เบอร์ 2)", price: 4, category: "อาหารสด", image: "" },
    { id: "M007", name: "ผงซักฟอกบรีส", price: 20, category: "ของใช้ในบ้าน", image: "" }
];
...
let isStoreOpen = true; // สถานะร้าน
let myLastOrders = [];  // เก็บรายการล่าสุดที่ลูกค้าสั่ง
...
function initBankQR() {
    ...
    // 🔴 1. ตรวจสอบ PromptPay ID ก่อน (ลำดับความสำคัญสูงกว่า)
    if (promptPayID) {
...
function savePromptPayID() {
    ...
    if (newID) {
        ...
        if (cleanID.length === 10 || cleanID.length === 13) {
            ...
            // ✅ ลบเฉพาะโค้ดที่ลบ bankQRID ออก เพื่อให้ ID รูปภาพยังคงอยู่
            // localStorage.removeItem('bankQRID'); // ❌ ไม่ต้องลบแล้ว
...
function checkAndUseOriginalQR() {
    // 1. ตรวจสอบว่ามี ID รูปภาพอยู่ในเครื่องหรือไม่ (ID นี้จะไม่ถูกลบเมื่อตั้ง PromptPay)
    const savedQR = localStorage.getItem('bankQRID');
    
    if (savedQR) {
        // 2. ถ้ามีรูปภาพเดิม -> สั่งลบ PromptPay ID ทิ้ง
        localStorage.removeItem('promptPayID');
        
        // 3. เรียกฟังก์ชันแสดงผล ซึ่งจะใช้ Bank QR ID แทน PromptPay ID ทันที
        initBankQR();
        ...
        // 4. ถ้าไม่มีรูปภาพอยู่ใน Local Storage เลย ให้แจ้งเตือนเหมือนเดิม
...
function initGlobalShortcuts() {
    document.addEventListener('keydown', function(event) {
        ...
        // 🟢 แก้ไข: ถ้ากำลังพิมพ์อยู่ในช่องสแกน หรือ ช่องรับเงิน -> ให้ Global หยุดทำงานทันที
        // ปล่อยให้ฟังก์ชันของช่องนั้นๆ (handleSearchKeydown) จัดการเอง
        ...
        if (key === '.' || code === 'NumpadDecimal' || key === 'Decimal' || key === 'Separator' || code === 'Period' || event.keyCode === 110 || event.keyCode === 190) {
            ...
            if (!paymentModal.classList.contains('hidden')) {
                ...
                // ถ้าอยู่หน้าปกติ ให้เด้งไปเข้าช่องค้นหา
...
function checkMode() {
    ...
    if (mode === 'customer') {
        ...
        // เปลี่ยนกล่องบาร์โค้ดด้านบน ให้กลายเป็นกล่องค้นหาข้อความ
        const searchInput = document.getElementById('searchInput');
        if (searchInput) {
...
function fetchMenu() {
    ...
    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: "getMenu" }) }) // ดึงหน้า Menu
    .then(res => res.json())
    .then(data => {
        if(data.result === 'success') {
            menuData = data.data; 
            filterMenu('All'); // แสดงผลหน้าแรก
            
            // --- เพิ่มบรรทัดนี้: โหลดสินค้าทั้งหมดมารอไว้สำหรับการค้นหา ---
            fetchMasterList(); 
        } 
    })
...
function fetchMasterList() {
    // ดึงข้อมูลสินค้าทั้งหมดมาเก็บไว้ใน masterData (ทำงานเบื้องหลัง)
    fetch(API_URL, { method: 'POST', body: JSON.stringify({ action: "getMasterList" }) })
    .then(res => res.json())
    .then(data => {
        if(data.result === 'success') {
            masterData = data.data;
            console.log("โหลดสินค้าทั้งหมดเรียบร้อย: " + masterData.length + " รายการ");
        }
    })
    .catch(err => console.error("โหลดสินค้าทั้งหมดไม่สำเร็จ", err));
}

// ฟังก์ชันกลางสำหรับจัดการเมื่อกด Enter (เพิ่มสินค้าตามราคา หรือ ยิงบาร์โค้ด)
function processSearchEnter() {
    ...
    if(trimVal) { 
        // ตรวจสอบว่าเป็นตัวเลข 1-4 หลักหรือไม่ (เช่น 20, 50, 100)
        // ถ้าใช่ -> เพิ่มเป็นสินค้าทั่วไป (ราคา)
        if (/^[0-9]{1,4}$/.test(trimVal)) { 
            addManualItem(parseInt(trimVal)); 
        } else { 
            // ถ้าไม่ใช่ (เช่น มีตัวอักษร, มีจุด, หรือยาวเกิน 4 หลัก) -> ค้นหาบาร์โค้ด
            scanBarcode(trimVal); 
        } 
    } else {
        // กรณีช่องว่างแล้วกด Enter -> เพิ่มจำนวนสินค้าตัวบนสุด (ถ้ามี)
...
function handleSearchKeydown(event) {
    ...
    if (event.key === '.' || event.code === 'NumpadDecimal' || event.keyCode === 110 || event.keyCode === 190) {
        ...
        if (paymentModal && !paymentModal.classList.contains('hidden')) {
            ...
            searchInput.value = ''; // เคลียร์ค่า
        }
        return;
    }

    // 3. จัดการ Enter (เรียกฟังก์ชันกลาง)
    if (event.key === 'Enter') {
        event.preventDefault(); 
        // เรียกฟังก์ชันกลางที่เราสร้างไว้ (processSearchEnter)
        processSearchEnter(); 
    }
    
    // 4. Backspace (ลบสินค้า)
...
function checkPaymentEnter(e) { 
    ...
    // ------------------------------------------------------------------
    // 🔴 3. บล็อกเลข 0 (Zero) ถ้าเป็นตัวแรกสุด
    // ------------------------------------------------------------------
    if (e.key === '0' || e.code === 'Numpad0') {
        // เช็คว่าช่องว่างอยู่หรือไม่ (ถ้าว่าง ห้ามพิมพ์ 0)
        if (e.target.value === '') {
            e.preventDefault();
            return;
        }
    ...
    // ------------------------------------------------------------------
    // ✅ Logic เดิม: การกด Enter เพื่อรับเงิน
    // ------------------------------------------------------------------
    if(e.key === 'Enter') { 
        e.preventDefault(); 
        ...
        // กรณีปกติ (มีการพิมพ์ยอดเงินเข้ามาแล้ว)
        if(!document.getElementById('btnConfirmPay').disabled) { 
            confirmPayment(); 
        } else { 
            playNotificationSound(); // เสียงเตือนถ้าเงินยังไม่ครบ
        } 
    } 
}
...
function scanBarcode(code) {
    // แปลงรหัสเป็น String และตัดช่องว่างหน้าหลังออกเพื่อความชัวร์
    const cleanCode = String(code).trim();
    const lowerCode = cleanCode.toLowerCase();

    // 1. ลองค้นหาจาก ID (บาร์โค้ด) ใน masterData ก่อน (ข้อมูลครบกว่า)
    let item = masterData.find(m => String(m.id).trim() === cleanCode);
    
    // 2. ถ้าไม่เจอใน master ให้หาใน menuData (เผื่อเป็นสินค้าใหม่ที่ยังไม่อัปเดตเข้า master)
    if (!item) {
        item = menuData.find(m => String(m.id).trim() === cleanCode);
    }

    // 3. ถ้ายังไม่เจอ ID ให้ลองหาจาก "ชื่อสินค้า" (แบบตรงตัวเป๊ะๆ)
    if (!item) {
    ...
    // --- ตรวจสอบผลลัพธ์ ---
    if(item) {
        // ✅ กรณีเจอสินค้า: เพิ่มลงตะกร้าเลย
        addItemToCart(item, "-"); 
        document.getElementById('searchInput').value = ''; 
        showToast(`เพิ่ม ${item.name} แล้ว`, 'success');
    } else {
        // ❌ กรณีไม่เจอสินค้าจริงๆ: ค่อยเปิดหน้าต่างเพิ่มสินค้า
        speak("ไม่มี");
...
function filterMenu(category) {
    ...
    // จัดการปุ่มกากบาท (Clear)
    const clearBtn = document.getElementById('clearSearchBtn');
    if(rawInput) clearBtn.classList.remove('hidden'); else clearBtn.classList.add('hidden');

    // --- ตรวจสอบว่าเป็นราคา (1-9999) หรือไม่ ---
    let searchText = rawInput;
    const isPrice = /^[0-9]{1,4}$/.test(rawInput) && parseInt(rawInput) > 0;
    
    if (isPrice) { searchText = ""; } 

    // --- 1. เลือกแหล่งข้อมูล (แก้ไขใหม่: รวม 2 แหล่งเมื่อค้นหา) ---
    let dataSource = [];
    
    if (searchText !== "") {
        // สร้าง Map เพื่อรวมข้อมูลและตัดตัวซ้ำ (ใช้ ID เป็นเกณฑ์)
        const combined = new Map();
        
        // 1. ใส่ข้อมูลจาก รายการสินค้า (Master) ลงไปก่อน
        if (masterData.length > 0) {
            masterData.forEach(m => { if(m.id) combined.set(String(m.id), m); });
        }
        
        // 2. ใส่ข้อมูลจาก Menu (หน้าแรก) ทับลงไป (เพื่อให้รายการใน Menu ไม่หาย)
        menuData.forEach(m => { if(m.id) combined.set(String(m.id), m); });
        
        // แปลงกลับเป็น Array เพื่อนำไปค้นหา
        dataSource = Array.from(combined.values());
        
        // กรณีฉุกเฉิน: ถ้าไม่มีข้อมูลเลย ให้ใช้ menuData อย่างเดียว
        if (dataSource.length === 0) dataSource = menuData;
        
    } else {
        // กรณีไม่ได้ค้นหา: ใช้ข้อมูลหน้า Menu ปกติ
        dataSource = menuData;
    }

    // --- 2. จัดการปุ่มหมวดหมู่ ---
    ...
    if (filtered.length === 0) {
        ...
        grid.innerHTML = filtered.map((item) => {
            ...
            if (hasImage) {
                ...
                // กรณีไม่มีรูป (แก้ไข HTML ให้ถูกต้อง ไม่หลุดกรอบ)
                ...
                <div class="p-4 pt-3 flex flex-col justify-between min-h-[110px]">
                    ...
                    <div class="flex justify-between items-end mt-1">
                        <div class="flex flex-col">
                             <span class="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-[-2px]">ราคา</span>
...
// --- เพิ่มฟังก์ชันใหม่สำหรับจัดการคลิก ---

// ฟังก์ชันค้นหาสินค้าจาก ID (ทั้งใน menuData และ masterData) แล้วส่งเข้าตะกร้า
function handleAddToCart(itemId) {
    // หาใน masterData ก่อน ถ้าไม่มีหาใน menuData
    let item = masterData.find(m => m.id == itemId) || menuData.find(m => m.id == itemId);
    if (item) {
        addItemToCart(item, "-"); // เรียกฟังก์ชันเดิมของคุณ
    } else {
        console.error("Item not found:", itemId);
    }
}

// ฟังก์ชันสำหรับปุ่มแก้ไข (รองรับการค้นหา)
function handleEditClick(itemId, e) {
    e.stopPropagation();
    // หา item จากทั้ง 2 แหล่ง
    let item = masterData.find(m => m.id == itemId) || menuData.find(m => m.id == itemId);
    if (item) {
...
function renderCart() {
    ...
    // --- กรณีตะกร้าว่าง ---
    if(cart.length === 0) {
        container.innerHTML = '<div class="h-full flex flex-col items-center justify-center text-gray-400 opacity-60"><i class="fas fa-cash-register text-6xl mb-4"></i><p>สแกนสินค้า หรือ กดปุ่มเพื่อเปิด QR รับเงิน</p></div>';
...
function openConfirmOrderModal() {
    ...
    if (isCustomerMode) {
        typeSelect.value = "ส่งเดลิเวอรี่";
        ...
        tableDiv.querySelector('label').innerText = "ชื่อลูกค้า / คิวที่";
        tableInput.placeholder = "เช่น 1 หรือ A";
    }
}
    ...
    const payload = { 
     ...
     fetch(API_URL, { method: 'POST', body: JSON.stringify(payload) }).then(res => res.json()).then(data => {
        if(data.result === 'success') {
            ...
            speak("ขออภัย ร้านปิดแล้วค่ะ");
            closeModal('confirmOrderModal'); 
            
        } else { 
            showCustomAlert('ผิดพลาด', 'ส่งออเดอร์ไม่สำเร็จ: ' + data.error, '<i class="fas fa-exclamation-circle text-red-500"></i>'); 
        }
        ...
        if (data.result === 'success') { 
            ...
            if (!isCustomerMode) {
                ...
                if (lastOrderCount !== -1 && waitingCount > lastOrderCount) { 
                    playNotificationSound(); 
                    showToast('มีรายการใหม่เข้ามา!', 'warning'); 
                }
...
function renderKitchen(orders) { 
    ...
    if(!orders || orders.length === 0) { 
        grid.innerHTML = '<div class="col-span-full flex flex-col items-center justify-center text-gray-300 py-20 animate-fade-in"><i class="fas fa-clipboard-check text-6xl mb-4"></i><p>ไม่มีรายการค้างส่ง</p></div>'; 
        return; 
    } 
            ...
            <div class="${headerBg} p-3 text-white flex justify-between items-center shadow-md">
                <div class="flex items-center gap-3">
                    <div class="bg-white/20 px-3 py-1 rounded-lg text-center min-w-[50px]">
                        <div class="text-[10px] font-bold opacity-80">คิว/โต๊ะ</div>
            ...
            <div class="p-4 flex-1 flex flex-col gap-3">
                ...
                <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    <div class="flex text-[10px] text-gray-500 font-bold bg-gray-100 px-3 py-2 border-b border-gray-200 uppercase tracking-wide">
                        ...
                        <div class="w-16 text-right">รวม</div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-3 py-2 border-t border-gray-200 flex justify-between items-center">
                        <span class="text-xs font-bold text-gray-500">ยอดสุทธิ</span>
                        <span class="text-lg font-extrabold text-blue-600">${order.totalPrice.toLocaleString()} ฿</span>
                    </div>
                </div>
            </div>
...
function openPayment(orderId) { 
    ...
    if(!currentPayOrder) {
        ...
        showCustomAlert('ผิดพลาด', 'ไม่พบข้อมูลออเดอร์นี้');
        return; 
    } 
...
function calcChange() { 
    ...
    if(mainChangeWrapper && mainChangeText) {
        if (received > 0) {
            ...
            if(mainTotalEl) {
                mainTotalEl.classList.remove('text-7xl');
                mainTotalEl.classList.add('text-4xl', 'translate-y-[-5px]');
            }
            ...
            if (change >= 0) {
                ...
                mainChangeText.innerText = "-" + Math.abs(change).toLocaleString();
                mainChangeText.classList.remove('text-green-600');
                mainChangeText.classList.add('text-red-500');
            }
        } else {
            ...
            if(mainTotalEl) {
                mainTotalEl.classList.remove('text-4xl', 'translate-y-[-5px]');
                mainTotalEl.classList.add('text-7xl');
            }
        }
    }
    ...
    if(received >= total && total > 0) {
        if(modalChangeBox) {
            ...
            modalChangeBox.innerHTML = `เงินทอน: <span class="text-green-600 text-5xl font-extrabold ml-2 drop-shadow-sm animate-heartbeat">${change.toLocaleString()}</span> <span class="ml-1 text-sm">฿</span>`;
        }
        
        if(change >= 0) { 
            clearTimeout(ttsTimer);
            ttsTimer = setTimeout(() => { speak("รับเงิน " + received + " บาท เงินทอน " + change + " บาท"); }, 800);
        }
        ...
        if(modalChangeBox && received > 0) {
             ...
             modalChangeBox.classList.remove('opacity-0', 'translate-y-2');
             modalChangeBox.innerHTML = `-: <span class="text-red-500 text-4xl font-extrabold ml-2 animate-heartbeat">${missing.toLocaleString()}</span> <span class="ml-1 text-sm">฿</span>`;
...
function confirmPayment() { 
    ...
    if (!currentPayOrder) {
        showCustomAlert('Error', 'ข้อมูลผิดพลาด กรุณาปิดหน้าต่างแล้วลองใหม่');
        return;
    }
    ...
    if (received === 0) {
        received = total;
        inputRec.value = total;
    }

    if (received < total) {
        showToast('ยอดเงินไม่ครบ', 'warning');
        playNotificationSound();
    ...
    const finalChange = received - total; 
    ...
    setTimeout(() => {
        const totalEl = document.getElementById('totalPrice');
        const changeWrapper = document.getElementById('changeWrapper');
        const mainScreenChange = document.getElementById('mainScreenChange');

        if(totalEl) {
            totalEl.innerText = total.toLocaleString() + " ฿";
            totalEl.classList.remove('text-7xl');
            totalEl.classList.add('text-4xl', 'translate-y-[-5px]');
        }

        if(changeWrapper && mainScreenChange) {
            mainScreenChange.innerText = finalChange.toLocaleString() + " ฿";
            mainChangeText = mainScreenChange; 
            
            mainScreenChange.classList.remove('text-red-500');
            mainScreenChange.classList.add('text-green-600');

            changeWrapper.classList.remove('hidden', 'opacity-0', 'translate-y-4');
            changeWrapper.classList.add('flex', 'opacity-100', 'translate-y-0');
        }
    }, 50);
...
function sendPaymentRequest(payload, isQuickPay) {
    ...
    .then(data => { 
        if(data.result === 'success') { 
            ...
            showCustomAlert('ผิดพลาด', 'บันทึกไม่สำเร็จ: ' + data.error);
        }
    })
    .catch(err => {
        showCustomAlert('Connection Error', 'ไม่สามารถติดต่อเซิร์ฟเวอร์ได้');
        console.error(err);
    });
}
    ...
    content.innerHTML = `
        <div class="p-4 pt-6 bg-white border-b border-gray-200 flex-shrink-0">
            <div class="text-center pb-3 border-b border-dashed border-gray-300">
                <h3 class="text-2xl font-extrabold text-gray-800 mb-1">ใบเสร็จรับเงิน</h3>
                <p class="text-xs text-gray-500 mb-2">บิล ID: ${bill.billId}</p>
            </div>

            <div class="pt-4 pb-2 text-xs space-y-1">
                <div class="flex justify-between">
                    <span class="text-gray-500">วัน/เวลา:</span>
                    <span class="font-bold text-gray-700">${dateStr} ${timeStr}</span>
                </div>
                <div class="font-bold text-gray-700 pt-2 border-t border-dashed">
                    <i class="fas fa-user-tag mr-2 text-blue-600"></i> ลูกค้า: ${customerName} ${customerPhone ? `(${customerPhone})` : ''}
                </div>
                ${customerAddress ? `
                    <div class="text-gray-500 text-xs">
                       <i class="fas fa-map-marker-alt text-red-500 mr-1"></i> ที่อยู่: ${customerAddress.replace(/\[ส่งที่:\s*|\]/g, '').trim()}
                    </div>
                ` : ''}
                ${bill.note ? `<div class="text-gray-500 pt-2 border-t border-dashed">หมายเหตุ: ${bill.note}</div>` : ''}
            </div>
        </div>
        ...
        <div class="p-4 flex-shrink-0 border-t border-gray-200 bg-white">
            <div class="pb-3 space-y-2 border-b border-gray-300">
                <div class="flex justify-between font-bold text-lg">
                    ...
                    <span class="text-2xl font-extrabold text-blue-600">${bill.total.toLocaleString()} ฿</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">รับเงิน</span>
                    <span class="font-bold text-gray-700">${parseFloat(bill.receive || bill.total).toLocaleString()} ฿</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">เงินทอน</span>
                    <span class="font-bold text-green-600">${parseFloat(bill.change || 0).toLocaleString()} ฿</span>
                </div>
            </div>
...
function executeDeleteBill() {
    ...
    setLoading('btnConfirmDeleteBill', true, 'กำลังลบ...');
    ...
    .finally(() => setLoading('btnConfirmDeleteBill', false, 'ยืนยันลบ'));
}
...
function printReceipt(bill) {
    ...
    const receiptHtml = `
        ...
        <body>
            <div class="header">
                <div class="store-name">ร้านเจ้พิน ขายของชำ</div>
                <div>ใบเสร็จรับเงิน</div>
            </div>
            
            <div class="divider"></div>
            <div style="font-size: 10px;">
                <div>วันที่: ${new Date(bill.date).toLocaleString('th-TH')}</div>
                <div>Bill ID: ${bill.billId}</div>
                <div>ลูกค้า: ${bill.table} (${bill.type})</div>
            </div>
            <div class="divider"></div>
...
function printReceiptFromIndex(index) {
    ...
    if(bill) {
        ...
        showCustomAlert('Error', 'ไม่พบข้อมูลบิล');
    }
}
...
function sendAddMenu(payload) {
     ...
     .then(d => { 
         if(d.result === 'success') { 
             ...
             showCustomAlert('ผิดพลาด', 'เพิ่มสินค้าไม่สำเร็จ: ' + d.error, '<i class="fas fa-exclamation-circle text-red-500"></i>'); 
         } 
     })
     .catch(err => { 
         showCustomAlert('Connection Error', 'ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้'); 
     })
...
function toggleStoreStatus() {
    ...
    .then(d => {
        if (d.result !== 'success') {
            isStoreOpen = !newStatus;
            updateStoreUI();
            showToast('เปลี่ยนสถานะไม่สำเร็จ', 'warning');
        } else {
...
function openMyRecentOrder() {
    ...
    if (myLastOrders && myLastOrders.items && myLastOrders.items.length > 0) { 
        ...
        const orderNote = myLastOrders.note || ''; 
        
        let addressString = "ไม่ระบุ";
        let remainingNote = orderNote;
        const addressMatch = orderNote.match(/\[ส่งที่:\s*(.*?)\]/); 
        
        if (addressMatch && addressMatch[1]) {
            addressString = addressMatch[1]; 
            remainingNote = orderNote.replace(addressMatch[0], '').trim(); 
        }
        ...
        let html = `
            <div class="text-center mb-6">
                <h3 class="text-xl font-extrabold text-blue-600 mb-1">ใบเสร็จรับเงิน (ออเดอร์ล่าสุด)</h3>
                <p class="text-xs text-gray-500">ขอบคุณที่ใช้บริการค่ะ</p>
            </div>
            ...
            <div class="bg-gray-50 p-4 rounded-xl mb-4 border border-gray-200 shadow-inner">
                <div class="font-bold text-sm text-gray-700 mb-2 border-b pb-2 border-dashed">
                   <i class="fas fa-user-tag mr-2 text-blue-600"></i> ผู้สั่ง: ${cName} 
                   <span class="text-xs text-gray-500">(${cPhone})</span>
                </div>
                
                <div class="text-sm text-gray-600">
                   <i class="fas fa-map-marker-alt mr-2 text-red-500"></i>
                   ที่อยู่จัดส่ง: <span class="font-bold">${addressString}</span>
                </div>

                ${remainingNote ? 
                    `<div class="text-xs text-gray-500 mt-2 pt-2 border-t border-dashed">หมายเหตุ: ${remainingNote}</div>` 
                    : ''
                }

                <div class="text-xs text-gray-500 mt-2 pt-2 border-t">
                   เวลาสั่ง: ${new Date(myLastOrders.timestamp).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                </div>
            </div>
        ...
        myLastOrders.items.forEach(item => { 
            ...
            html += `
                <div class="flex justify-between items-center bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                    ...
                    <div class="font-bold text-blue-600">${itemTotal.toLocaleString()} ฿</div>
                </div>
            `;
        });
        ...
        html += `</div>
            <div class="mt-4 pt-4 border-t border-dashed border-gray-300 flex justify-between items-center">
                <span class="text-gray-600 font-bold">รวมทั้งหมด</span>
                <span class="text-2xl font-bold text-blue-600">${total.toLocaleString()} ฿</span>
            </div>
            <div class="mt-6 text-center">
                <p class="text-green-600 font-bold text-sm mb-2"><i class="fas fa-check-circle"></i> ทางร้านได้รับออเดอร์แล้ว</p>
                <button onclick="closeModal('myOrderModal')" class="bg-gray-800 text-white w-full py-3 rounded-xl font-bold hover:bg-gray-900 transition">ปิดหน้าต่าง</button>
            </div>
        `;
        content.innerHTML = html;
    } else {
         content.innerHTML = '<div class="text-center py-10"><i class="fas fa-shopping-basket text-4xl text-gray-300 mb-2"></i><p class="text-gray-400">ยังไม่มีรายการที่สั่งล่าสุด</p></div>';
    }
    modal.classList.remove('hidden');
}
...
document.getElementById('customerIdentityForm').addEventListener('submit', function(e) {
    ...
    if(!name || !phone || !houseNo || !soi) { 
        showToast('กรุณากรอกข้อมูลให้ครบถ้วน', 'warning');
        return;
    }
    if(phone.length < 9 || isNaN(phone)) {
        showToast('เบอร์โทรศัพท์ไม่ถูกต้อง', 'warning');
        return;
    }
...
document.getElementById('loginForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        if(data.result === 'success') {
            ...
            document.getElementById('loginErrorMsg').innerText = data.error || 'รหัสผ่านไม่ถูกต้อง';
    ...
    .catch(err => {
        document.getElementById('loginErrorMsg').innerText = 'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้';
        errorBox.classList.remove('hidden');
    })
    .finally(() => {
...
function submitChangePassword() {
    ...
    if (!phoneVal) { alert("กรุณาระบุเบอร์โทรศัพท์"); return; }
    if (!newPass) { alert("กรุณากรอกรหัสผ่านใหม่"); return; }
    ...
    .then(data => {
        if(data.result === 'success') {
            ...
            alert('เปลี่ยนรหัสไม่สำเร็จ: ' + data.error);
        }
    })
    .catch(err => { alert('Error: ' + err); })
    .finally(() => { setLoading('btnSubmitChangePass', false, 'บันทึกรหัสผ่านใหม่'); });
}
        ...
        if (el) {
            el.addEventListener('input', function(e) {
                ...
                if (thaiRegex.test(this.value)) {
                    ...
                    showToast('รหัสผ่านต้องเป็นภาษาอังกฤษ/ตัวเลขเท่านั้น', 'warning');
...
function executeExportPDF() {
    ...
    if (!start || !end) {
        showToast('กรุณาเลือกวันที่ให้ครบ', 'warning');
        return;
    }
    ...
    .then(data => {
        if (data.result === 'success') {
            ...
            showCustomAlert('ผิดพลาด', 'สร้าง PDF ไม่สำเร็จ: ' + data.error);
        }
    })
    .catch(err => {
        console.error(err);
        showCustomAlert('Connection Error', 'ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้');
    })
    .finally(() => {
        setLoading('btnDoExport', false, 'ดาวน์โหลด PDF');
    });
}
...
function syncSearch(val) {
    ...
    const btnClear = document.getElementById('btnClearFloat');
    if(val) {
